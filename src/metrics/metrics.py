...
class CFMetrics:
    ...
    def __init__(
        ...
        # Convert y_target to class indices if it's one-hot encoded
        if self.y_target_covered.ndim == 2:
...
class CFMetricsNoPlausibility:
    ...
    def __init__(
        ...
        # convert everything to torch tensors if not already
        self.X_cf = self._convert_to_numpy(X_cf)
        self.y_target = self._convert_to_numpy(y_target)
        self.X_train = self._convert_to_numpy(X_train)
        self.y_train = self._convert_to_numpy(y_train)
        self.X_test = self._convert_to_numpy(X_test)
        self.y_test = self._convert_to_numpy(y_test)
    ...
    def feature_distance(
        ...
        Compute the distance metric.
        Args:
            continuous_metric (str, optional): Distance metric to use for continuous features. Defaults to "euclidean".
            categorical_metric (str, optional): Distance metric to use for categorical features. Defaults to "jaccard".
            X_train (np.ndarray, optional): Training data for normalization. Defaults to None.
    ...
    def calc_all_metrics(self) -> dict:
        """
        Calculate all metrics excluding plausibility-related metrics.
